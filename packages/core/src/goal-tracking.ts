...
export function calculatePriceChange(oldPrice: number, newPrice: number): PriceChange {
  ...
  const direction =
    absoluteChange > 0 ? 'INCREASE' : absoluteChange < 0 ? 'DECREASE' : 'UNCHANGED';
...
export function checkSignificantPriceChange(
  ...
  if (
    ...
    Math.abs(change.percentageChange) < threshold
  ) {
    return null;
  }
...
function normalizeSingleSeparatorNumber(value: string, separator: ',' | '.'): string {
  ...
  const looksLikeThousands = hasRepeatedSeparators || lastPart.length === 3;
