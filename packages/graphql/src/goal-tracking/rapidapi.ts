...
function normalizeProductResult(item: UnknownRecord): ProductSearchResult | null {
  ...
  const image =
    stringFrom(item.image) ?? stringFrom(item.thumbnail) ?? stringFrom(item.product_image) ?? null;
  const source =
...
function matchBySource(
  ...
  return (
    products.find(
      (product) => normalizeComparableText(product.source) === normalizedSource && product.price > 0
    ) ?? null
  );
}
...
export function buildPriceRefreshMessage(
  result: ProductSearchResult | null,
  currentPrice: unknown
): string | null {
  if (!result) {
    return 'Seçili ürün için güncel fiyat bulunamadı. Daha sonra tekrar deneyebilirsin.';
  }

  const previousPrice = moneyToNumber(currentPrice);
  if (previousPrice === result.price) {
    return 'Güncel fiyat değişmedi.';
  }
