...
Bu workspace npm ile de çalışabilir:

| Komut | Açıklama |
|---|---|
| `npm install` | Workspace bağımlılıklarını kur |
| `npm run type-check` | TypeScript proje referanslarını kontrol et |
| `npm test` | Vitest testlerini çalıştır |
| `npm run db:generate` | Prisma client üret |
| `npm run dev` | Standalone GraphQL Yoga server'ı başlat |
